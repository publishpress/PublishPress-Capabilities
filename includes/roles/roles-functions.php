...
function admin_roles_page_load()
{
    $plugin_name = 'capsman';
    //enqueue styles
    wp_enqueue_style($plugin_name, plugin_dir_url(__FILE__) . 'css/pp-roles-admin.css', array(), PUBLISHPRESS_CAPS_VERSION, 'all');

    //enqueue scripts
    wp_enqueue_script($plugin_name . '_table_edit', plugin_dir_url(__FILE__) . 'js/pp-roles-admin.js', array('jquery'), PUBLISHPRESS_CAPS_VERSION, false);
    ...
    pp_roles()->admin->get_roles_list_table();

    //Handle actions
    pp_roles()->admin->handle_actions();
