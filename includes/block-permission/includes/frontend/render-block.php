...
namespace BlockPermission\Frontend;
...
function has_visibility_settings( $block ) {
	if ( isset( $block['attrs']['BlockPermission'] ) ) {
		return true;
	}
...
function render_with_visibility( $block_content, $block ) {
	...
	if (
		...
		$visibility_test = apply_filters(
			'ppc_block_permission_visibility_test',
			$visibility_test,
			$settings,
			$block
		);
...
// Run our tests.
require_once BLOCK_PERMISSION_ABSPATH . 'includes/frontend/permission-tests/hide-block.php';
require_once BLOCK_PERMISSION_ABSPATH . 'includes/frontend/permission-tests/permission-by-role.php';

// Require utlity functions for tests.
require_once BLOCK_PERMISSION_ABSPATH . 'includes/utils/is-control-enabled.php';
